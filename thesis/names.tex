...
{[[ G, x : [A] r |- B =>- [x/y] t ; D, x : [A] s' ]]}
  ...
  \inferrule*[right=\subUnboxName]
    {[[ G, x2 : [A] r |- B =>- t ; D, x2 : [A] s && 0 <= s]]}{[[G, x1 : [] r A |- B =>- let [x2] = x1 in t ; D ]]}
    }
...
\newcommand*{\fSubUnboxRule}{
    ...
    {[[G ; {O, x2 : [A] r} async |- B =>- t ; D, x2 : [A] s && 0 <= s]]}{[[G; {O, x1 : []
...
\newcommand*{\fSubGrVarRule}{
    ...
    {[[exists ex]]}{[[G ; {x : [A] r} sync |- A =>- x ; G, x : [A] s]]}
}
...
\newcommand*{\fAddPairElimRuleNoLabel}{
    ...
    {[[G ; O, x1 : A, x2 : B |- C =>+ t2 ; D, x1 : A, x2 : B]]}{[[G ; O, x3 : A * B |- C =>+ letpair x1 x2
  = x3 in t2 ; D, x3 : Tup A B ]]}
}
