...
DISCUSSION OF GRADED LINEAR VS GRADED GOES HERE
...
For this work, we consider Granule~\citep{DBLP:journals/pacmpl/OrchardLE19} to be
...
permeate the type system à la ~\citet{petricek2014coeffects}. This
...
graded modalities into the function types à la Idris 2 and Linear Haskell,
...
\begin{array}{r|c|c}
      & \text{ Graded} + \text{Linear } & \text{ Graded}\\
      \hline
      \text{Chapter~\ref{chapter:background}} & \multimap,\ \Box_r & \rightarrow,\ \Box_r \\
      ...
      \text{Chapter~\ref{chapter:extended}} & & \rightarrow,\ \Box_r,\ \text{ADTs},\ \text{Recursion}
\end{array}
\end{align*}
...
\begin{itemize}
    ...
    calculi based on these approaches. This paper constitutes a large part of
    ...
    In Chapter~\ref{chapter:background} and subsequently, we make reference to the 
    disparity between Granule's $\Box$ modality and Linear Logic's !, and how ! can 
    be recovered in Granule through the use of a grade-level operator defined 
    in this paper.  
\end{itemize}
