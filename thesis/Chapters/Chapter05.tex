...
% The logic behind this grade
% calculation is illustrated in the soundness proof for $\rightarrow_\textsc{L}$. 
...
% The total usage of $[[ x1 ]]$ 
% in the application $[[ x1 t2 ]]$ is thus given by $s2 * q * s3$. 



% In the proof, this allows us to construct a typing derivation for the
% application $[[ x1 t2 ]]$. We make use of the distributive property of semirings
% along with the commutativity of $+$ and unitality of $1$ such that $ = (s_2 \cdot 1) + (s_2 \cdot q \cdor s_3) = s_2 \cdot q \cdot s_3 $ to yield the typing
% context $[[ D2 ]], x_1 :_{1 \cdot (q \cdot s_3)} A^q \rightarrow B$, $()$
