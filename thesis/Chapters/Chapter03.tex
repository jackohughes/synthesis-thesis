...
integrate these types into the design of a synthesis tool, using resource
constraints to prune the search space of programs. 
...
then the synthesis tool must synthesise a $t$ which uses $x$ exactly once.
...
number of derelictions that can be used so that the grade $r$ is not
exceeded.
...
for soundness (i.e. that synthesised programs are well-typed); we discuss
soundness further in Section~\ref{sec:linear-base-sub-sound}.
...
\end{align*}
\subsubsection{Sums}
The right rules for sum types, \subSumIntroLname\ and \subSumIntroRname, are straightforward:
\begin{align*}
  \subSumIntroL
\;\;\;
  \subSumIntroR
...
single usage which will satisfy the incoming grade. We apply this pruning
condition at the synthesis of terms which are binders. Therefore, synthesised
...
straightforward, by induction. Again, we consider the $\multimap_\textsc{R}^+$ rule. By induction we have that:
%
\begin{align*}
  [[ (G, x : A) - D |- t : B ]]
\end{align*}
...
restricts synthesis to generating programs in $\beta$-normal form.
...
\begin{itemize}
  ...
  \item We have exposed an asynchronous connective at which point we switch back to applying asynchronous rules.
\end{itemize}
...
assumption from $\Gamma$ to $\Omega$ if the type is not left
asynchronous. 
...
function equations and to avoid excessive use of let bindings: 
\begin{granule}
