...
This issue of resource non-determinism has been encountered before in the
...
(LA) edge[loop above] node{$\otimes_{\textsc{L}}$, $\oplus_{\textsc{L}}$, \\ $\mathsf{Unit}_\textsc{L}$, $\textsc{Der}$, \\ $\Box_{\textsc{L}}$, $\Uparrow_{\textsc{L}}$ } (LA)
% (LA) edge[loop right] node{$\Uparrow_{L}$} (LA)
(LA) edge[left, bend right] node{$\otimes_{\textsc{L}}$, $\oplus_{\textsc{L}}$, \ \ \ \\ $\mathsf{Unit}_\textsc{L}$, $\textsc{Der}$, \ \ \ \ \\ $\Box_{\textsc{L}}$, $\Uparrow_\textsc{L}$ \ \ \ \ \\  } (F)
